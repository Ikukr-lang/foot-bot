...
if not BOT_TOKEN or not PROVIDER_TOKEN:
    raise ValueError("BOT_TOKEN or PROVIDER_TOKEN not set!")
...
async def get_new_tickets_count():
    async with aiosqlite.connect(DB_NAME) as db:
        return (await (await db.execute("SELECT COUNT(*) FROM support_tickets WHERE status = 'new'")).fetchone())[0]
...
async def get_all_slots():
    async with aiosqlite.connect(DB_NAME) as db:
        return await (await db.execute("SELECT slot, event_text, is_published FROM matches ORDER BY slot")).fetchall()
...
async def slots_keyboard():
    ...
    kb = [[InlineKeyboardButton(text=f"✅ Слот {i} | {occupied[i]}" if i in occupied else f"□ Слот {i} — свободен", callback_data=f"add_to_slot_{i}")] for i in range(1, 21)]
...
def get_max_matches(sub_type: str, weekday: int) -> int:
    limits = {
        "gold_28": [5,5,5,5,10,20,20],
        "gold_14": [5,5,5,5,8,17,17],
        "silver_28": [3,3,3,3,5,12,12],
        "silver_14": [3,3,3,3,4,10,10],
        "free": [1,1,1,1,2,2,2]
    }
    return limits.get(sub_type, limits["free"])[weekday]
...
async def get_subscription(user_id: int):
    async with aiosqlite.connect(DB_NAME) as db:
        row = await (await db.execute("SELECT subscription, sub_end FROM users WHERE telegram_id = ?", (user_id,))).fetchone()
        if not row:
            return "free", None
        sub, end = row
...
async def get_daily_count(user_id: int) -> int:
    today = moscow_today()
    async with aiosqlite.connect(DB_NAME) as db:
        row = await (await db.execute("SELECT count FROM daily_usage WHERE telegram_id=? AND date=?", (user_id, today))).fetchone()
...
async def get_sub_counts():
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        counts = {}
        for sub in subs:
            counts[sub] = (await (await db.execute("SELECT COUNT(*) FROM users WHERE subscription = ? AND (sub_end > ? OR sub_end IS NULL)", (sub, now_str))).fetchone())[0]
        counts['free'] = (await (await db.execute("SELECT COUNT(*) FROM users WHERE subscription = 'free' OR sub_end <= ?", (now_str,))).fetchone())[0]
    return counts

async def get_all_processed_users():
    now_str = moscow_now().replace(tzinfo=None).isoformat()
    async with aiosqlite.connect(DB_NAME) as db:
        rows = await (await db.execute("SELECT telegram_id, username, subscription, sub_end FROM users ORDER BY telegram_id")).fetchall()
        processed = []
...
async def get_matching_users(search: str):
    now_str = moscow_now().replace(tzinfo=None).isoformat()
    async with aiosqlite.connect(DB_NAME) as db:
        rows = await (await db.execute("SELECT telegram_id, username, subscription, sub_end FROM users WHERE username LIKE ? ORDER BY telegram_id", (f"%{search}%",))).fetchall()
        processed = []
        for uid, uname, sub, end in rows:
            if end and end < now_str:
                sub = 'free'
...
async def cmd_start(message: Message):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        row = await (await db.execute("SELECT subscription FROM users WHERE telegram_id=?", (user_id,))).fetchone()
        if row is None:
            sub_type = "silver_28"
...
async def admin_subscriptions(callback: CallbackQuery):
    ...
    text = "<b>Подписки:</b>\n" + "\n".join(f"{get_sub_name(sub)}: {counts.get(sub, 0)}" for sub in ['silver_14', 'silver_28', 'gold_14', 'gold_28'])
...
async def show_all_users(callback: CallbackQuery):
    ...
    kb = [[InlineKeyboardButton(text=f"{uid} @{uname} ({get_sub_name(sub)})", callback_data=f"manage_user_{uid}")] for uid, uname, sub in users]
...
async def gift_subscription(callback: CallbackQuery):
    ...
    start_date = datetime.fromisoformat(current_end) if current_sub == sub_type and current_end else moscow_now().replace(tzinfo=None)
...
async def select_slot_for_match(callback: CallbackQuery, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        if await (await db.execute("SELECT 1 FROM matches WHERE slot=?", (slot,))).fetchone():
...
async def save_match_file(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute("INSERT INTO matches (slot, event_text, file_id, is_published) VALUES (?, ?, ?, 0)", (data["slot"], data["event_text"], message.document.file_id))
        await db.commit()
    await state.clear()
...
async def admin_show_support(callback: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        rows = await (await db.execute("SELECT id, telegram_id, username, text FROM support_tickets WHERE status = 'new'")).fetchall()
        if not rows:
    ...
    for ticket_id, telegram_id, username, text in rows:
...
async def save_support_reply(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        user_id = (await (await db.execute("SELECT telegram_id FROM support_tickets WHERE id = ?", (ticket_id,))).fetchone())[0]
...
async def show_matches(message: Message):
    ...
    if sub == "free" and not subscribed:
        ...
        return
    async with aiosqlite.connect(DB_NAME) as db:
        rows = await (await db.execute("SELECT id, slot, event_text FROM matches WHERE is_published=1 ORDER BY slot")).fetchall()
    if not rows:
        await message.answer("Пока нет опубликованных матчей.")
        return
    kb = []
    for mid, slot, text in rows:
        already = await (await db.execute("SELECT 1 FROM user_match_access WHERE telegram_id=? AND match_id=?", (message.from_user.id, mid))).fetchone()
        display = f"{slot}. {text}"
        kb.append([InlineKeyboardButton(text=f"✅ {display}" if already else display, callback_data=f"match_{mid}" if not already else f"already_{mid}")])
...
async def give_match_file(callback: CallbackQuery):
    ...
    if opened >= max_m:
        ...
        return
    async with aiosqlite.connect(DB_NAME) as db:
        row = await (await db.execute("SELECT file_id FROM matches WHERE id=?", (match_id,))).fetchone()
        if not row:
            await callback.answer("Файл не найден")
            return
        file_id = row[0]
        cur = await db.execute("INSERT OR IGNORE INTO user_match_access (telegram_id, match_id) VALUES (?, ?)", (callback.from_user.id, match_id))
        if cur.rowcount == 0:
...
async def save_support(message: Message, state: FSMContext):
    username = message.from_user.username
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute("INSERT INTO support_tickets (telegram_id, username, text) VALUES (?, ?, ?)", (message.from_user.id, username, message.text))
        await db.commit()
    if ADMIN_ID:
