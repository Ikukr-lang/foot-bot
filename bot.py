...
# ================== НАСТРОЙКИ ==================
TOKEN = "ВАШ_ТОКЕН_ОТ_BOTFATHER"  # Замени!
bot = Bot(token=TOKEN)
dp = Dispatcher()

# Таблица конвертации рейтингов в числа (Hattrick стандарт)
LEVELS = {
        ...
        if lvl_name in text:
            ...
            for sub_name, add in SUBS.items():
                if sub_name in text:
                    val += add
                    break
            # если "high" без "very" — примерно +0.25
...
def calculate_match_prob(home_ratings: dict, away_ratings: dict, poss45: float, poss90: float):
    # Усредняем possession (соединяем показанное + расчёт по Mid)
    ...
    poss_avg = max(0.3, min(0.7, poss_avg))  # ограничиваем реализмом

    chances_home = 15 * poss_avg
    chances_away = 15 * (1 - poss_avg)

    # Среднее преимущество атаки
    ...
    # Вероятность гола (42.5% базовая + бонус от преимущества)
    ...
    # Poisson вероятности (аналитически, без тяжёлых библиотек)
...
def parse_match(url: str):
    ...
    # Рейтинги — ищем все текстовые рейтинги (самый надёжный способ)
    ...
    # Простой способ: берём первые 14 значимых рейтингов (7 home + 7 away)
    nums = []
    for lvl, sub in ratings[:20]:  # запас
        num = text_to_rating(f"{lvl} {sub or ''}")
        if num > 1: nums.append(num)

    if len(nums) < 14:
        raise ValueError("Не удалось распарсить рейтинги. Проверьте ссылку или обновите парсер.")

    # Распределяем (порядок в отчёте обычно GK, Def L/C/R, Mid, Att L/C/R)
    ...
    away = {"gk": nums[8], "def_l": nums[9], "def_c": nums[10], "def_r": nums[11],
            "mid": nums[12], "att_l": nums[13], "att_c": nums[14] if len(nums)>14 else nums[6], "att_r": nums[15] if len(nums)>15 else nums[7]}
...
async def start(message: Message):
    await message.answer("Отправь ссылку на матч Hattrick (например https://www.hattrick.org/Club/Matches/Match.aspx?MatchID=123456789)\nЯ посчитаю точные % на основе рейтингов 1-й и последней минуты + possession.")

@dp.message(F.text.regexp(r"hattrick\.org.*MatchID=\d+"))
async def process_match(message: Message):
    url = message.text.strip()
    await message.answer("⏳ Парсю матч и считаю вероятности...")
    ...
    try:
        ...
        text = (f"✅ Анализ завершён!\n\n"
                ...
                f"Использовал: рейтинги (1-я минута → конец) + possession 45'/90' + полный механизм ME.")
        await message.answer(text)
    except Exception as e:
        await message.answer(f"❌ Ошибка: {str(e)}\n\nВозможно, страница требует входа или HTML изменился. Пришли скриншот отчёта — доработаю парсер за 2 минуты.")
